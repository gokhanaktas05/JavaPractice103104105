...
*/
    ...
    static void testsonucu(char[][] answers, char[] keys) {
        for (int i = 0; i < answers.length; i++) {  //Her bir öğrenciyi dolaştık... i öğrenciler
            int sayac=0;   // her bir öğrencinin doğrucevap sayısı
            for (int j = 0; j < answers[i].length; j++) { // her öğrencinin cevaplarına bakar...j cevaplar

                if (answers[j][i] == keys[j]) {
                    sayac++;
                }
            }
            System.out.println(i +1 + "no lu öğrencinin doğru cevap sayısı"+ sayac+ "kadardır.");
        }
